...
asymmetric_weights <- function(graph, perplexity = 4, tol = 1e-5, max_iter = 100) {
  if (!igraph::is.igraph(graph)) stop("Input must be an igraph object")
  if (igraph::is.directed(graph)) stop("Input graph must be undirected")
  ...
  for (i in seq_along(nodes)) {
    ...
    dists <- sapply(neighbors_i, function(nj) {
      e_id <- igraph::get.edge.ids(graph, c(ni, nj), directed = FALSE)
