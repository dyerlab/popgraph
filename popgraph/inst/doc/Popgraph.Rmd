...
```{r}
nodes <- to_SpatialPoints(lopho)
nodes
edges <- to_SpatialLines(lopho)
edges
```
...
```{r, fig.width=8}
plot( alt )
plot( edges, add=TRUE, col="#555555" )
plot( nodes, add=TRUE, col="black", cex=1.5 )
plot( nodes, add=TRUE, col=V(lopho)$color, pch=16, cex=1.5 )
```
...
```{r}
df.nodes$Elevation <- extract( alt, nodes )
summary(df.nodes)
```
...
```{r}
plot(alt)
plot(edges[3],add=TRUE,col="red",lwd=3)
```
...
```{r,message=FALSE}
require(ggplot2)
Elevation <- extract( alt, edges[3] )[[1]]
Latitude <- seq(26.59,29.03,length.out=length(Elevation))
qplot( Latitude, Elevation, geom="line" )
```

There is a lot of ways that you can play with this functionality.  This should get you going though.
